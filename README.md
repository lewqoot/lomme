...
Pull request и каждый push в `main` проходят единый обязательный CI: чистая
PostgreSQL 16, все миграции с нуля, typecheck, lint, тесты, обе сборки и smoke
API/worker. CI сохраняет один артефакт с точным Git SHA и SHA-256 веб- и
серверного bundle. `/healthz`, `/readyz` и итоговая строка worker показывают
роль, Git SHA, версию приложения и версию схемы.

Перед ручным production-релизом запусти `npm run release:prepare`. Команда
откажется работать с незакоммиченным деревом, заново установит lockfile,
повторит полный CI-прогон и создаст `dist-release/release-manifest.json`.
Выкатывать API и worker можно только из одного SHA; успех подтверждается не
статусом очереди Railway, а `/readyz` API и строкой `worker_batch_complete` с
этим же SHA. Миграции в релизе должны оставаться обратно совместимыми с
предыдущей версией API до завершения обеих выкладок.
