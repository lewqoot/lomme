...
describe('Telegram initData', () => {
  it('принимает корректно подписанные свежие данные', () => {
    ...
    expect(validateTelegramInitData(createTelegramInitDataForTest(identity, token, now), token, 300, now)).toEqual(identity)
  })
  ...
  it('принимает текущий формат Mini App с дополнительной подписью Telegram', () => {
    ...
    expect(validateTelegramInitData(initData, token, 300, now)).toEqual(identity)
  })
