...
function AccountsPage({ data, workspace, onSelect, onResetScope, onRefresh, notify, onClose }: ActionProps & AccountSwitchProps & { workspace?: AppSnapshot['workspaces'][number]; onClose(): void }) {
  ...
  return <div className="accounts-screen">
    ...
    <button className={`all-accounts-row${data.activeAccountId === null ? ' active' : ''}`} type="button" disabled={select.isPending} onClick={() => select.mutate(null)}><strong>Все счета · {workspace?.name || 'пространство'}</strong><span>{money(total)}</span>{data.activeAccountId === null && <Check />}</button>
